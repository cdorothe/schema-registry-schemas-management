...
import com.chdor.schema_registry.schemas.management.json.model.TVShowPojo;
...
public class GenerateJsonSchema {
	...
	public static void generateJsonSchemaWithConfluentSchemaRegistry() throws Exception {
		...
		JsonSchema jsonSchema = JsonSchemaUtils.getSchema(new TVShowPojo());
		...
		jsonSchema = JsonSchemaUtils.getSchema(new TVShowPojo(), SpecificationVersion.DRAFT_7, false,
	...
	public static void generateJsonSchemaWithJacksonJsonSchemaGenerator() {
		...
		// Defines the array default format
		...
		// If rendering array and type is instanceOf class in this set, then we add
		// 'uniqueItems": true' to schema - See // https://github.com/jdorn/json-editor
		// for more info
		...
		// Suppliers in this map can be accessed using
		// @JsonSchemaInject(jsonSupplierViaLookup = "lookupKey")
		...
		// Used to match against different validation-groups
		// (javax.validation.constraints)
		Class<?>[] javaxValidationGroups = new Class<?>[0];
		// Select the $schema draft version
		...
		// Defines the Title Schema. Disable the camelCase transformation
		String title = "TVShow";
		...
		JsonNode schemaJsonNode = jsonSchemaGenerator.generateJsonSchema(TVShowPojo.class, title, description);
		//System.out.println("JSON Schema generated from POJO with Jackson jsonSchema Generator:\n"
		...
		// Display the JsonSchema inferred from our TVShowPojo class
		System.out.println("JSON Schema generated converts into JsonSchema:\n" + new JSONObject(jsonSchema.toString()).toString(2)); 
